...
public class Knapsack01 {
    ...
    private static void MaxKnapsack01DPMatrix(int[] val, int[] wt, int capacity) {
        int[][] dp = new int[wt.length][capacity + 1];
        for (int i = 0; i < wt.length; i++) {
            for (int j = 1; j <= capacity; j++) {
                if (j - wt[i] >= 0) { // ith item can be included
                    dp[i][j] = Math.max(i > 0 ? dp[i - 1][j] : 0, val[i] + i > 0 ? dp[i - 1][j - wt[i]] : 0);
                } else { // ith item can not be included
